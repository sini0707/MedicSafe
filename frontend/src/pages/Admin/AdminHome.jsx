...
const AdminHome = () => {
  ...
  useEffect(() => {
    const fetchMonthlyData = async () => {
      try {
        ...
        // Only create the new chart if the canvas element exists
        const ctx = document.getElementById("chartBar");
        if (ctx) {
          // If chartBar is already set, update the chart data
          if (chartBar) {
  ...
  useEffect(() => {
    if (!yearlyBookings.length) {
      return; // Don't proceed if yearlyBookings is empty
    }
  ...
  return (
    ...
    <div className="p-4 xl:ml-80">
      ...
      <div className="mt-12">
       ...
       <h4 className='my-5 text-xl font-bold'>Count : <span className='text-blue-700'>{users&&users.length}</span></h4>
            </div>
        ...
        <div className='md:flex md:justify-evenly my-5 text-xl font-bold'>
           <h4>Count :<span className='text-blue-700'>{doctors&&doctors.length}</span></h4>
        ...
        <h4 className='my-5 text-xl font-bold'>Count : <span className='text-blue-700'>{booking&&booking.length}</span></h4>

        {/* <h4 className='my-5 text-xl font-bold'>Total Amount: <span className='text-blue-700'>{total&&total.length}</span></h4> */}
            </div>
  ...
  {/* Pie Chart */}
  <div className="relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md">
  <canvas id="chartPie" style={{ maxWidth: '400px', margin: '0 auto' }}></canvas>
  </div>
</div>
