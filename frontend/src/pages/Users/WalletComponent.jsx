...
const WalletComponent = () => {
    ...
    useEffect(() => {
        const fetchWallet = async () => {
            try {
                ...
                const response = await fetch(`${baseURL}/users/get-wallet?id=${user._id}`, {
                    method: 'POST',
                    headers: {
    ...
    return (
        <div>
            <div className="font-regular relative block w-full max-w-screen-md rounded-lg bg-green-500 px-4 py-4 text-base text-white" data-dismissible="alert">
            <p className='text-center text-lg font-bold text-black '>Wallet component</p>
            ...
            {wallet && (
                <div>
                    <h2 className="text-lg font-bold">Wallet Balance: <span style={{ color: 'black' }}></span>${wallet.balance}</h2> 
